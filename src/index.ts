...
export class Logger implements ILogger {
    ...
    custom(message: string, type: string) {
        if (this.customLevels[type] != null || this.customLevels[type] != undefined) {
    ...
    private formatMessage(message: string, type: string, useColor = true) {
        if (useColor && this.config[type] && this.config[type].color && chalk[this.config[type].color]) {
            return this.format.replace("%t", chalk[this.config[type].color](type.toUpperCase())).replace("%d", chalk.bold.black(getCurrentTime())).replace("%m", message)
        } else {
            return this.format.replace("%t", type.toUpperCase()).replace("%d", getCurrentTime()).replace("%m", message)
        }
    }
}